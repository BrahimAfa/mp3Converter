...
app.get('/playlist/:id',async (req,res)=>{
    ...
    let filterplaylist = ['--max-downloads',max];

    if (start) filterplaylist = [...filterplaylist,"--playlist-start",start]
    if (end) filterplaylist = [...filterplaylist,"--playlist-end",end]
    
    try{
        yt_dl= execa('youtube-dl',[...filterplaylist, "-x","--no-progress","-o","./mp3/%(title)s.%(ext)s","--no-warnings", "--audio-format","mp3",`https://www.youtube.com/playlist?list=${req.params.id}`])
      //  youtube-dl -x -o './mp3/%(title)s.%(ext)s' --no-warnings --audio-format mp3 https://www.youtube.com/playlist?list=PLETIo5u_JSiOrahkO8xY5tvzh8O4PLJA1
        yt_dl.stdout.pipe(stdoutFile);
